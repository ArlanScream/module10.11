...
const display = () => {
  ...
  for (let i = 0; i < fruits.length; i++) {
    ...
    if (fruits[i].color == 'светло-зеленый') {
...
const comparationColor = (a, b) => {
  const priorColor = ['желтый', 'зеленый', 'розово-красный', 'фиолетовый', 'светло-коричневый'];
  const color1 = priorColor.indexOf(a.color);
  const color2 = priorColor.indexOf(b.color);
...
const sortAPI = {
  ...
  quickSort(arr, comparation) {
    ...
    for (let i = 0; i < arr.length; i += 1) {
      ...
      if (comparation(arr[i], currentEl)) {
        more.push(arr[i]);
      } else {
        less.push(arr[i])
      }
